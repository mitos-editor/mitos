...
fn main() {
    ...
    #[cfg(windows)]
    windows_rc::link_icon_in_windows_exe("../../contrib/mitos-256p.ico");
}
                    ...
                    for line in lines {
                        if line.trim().starts_with("KitsRoot") {
                            let kit: String = line
                                ...
                                .collect();
                            let path = PathBuf::from(&kit);
                            let rc = if cfg!(target_arch = "x86_64") {
                                path.join(r"bin\x64\rc.exe")
                            } else {
                                path.join(r"bin\x86\rc.exe")
                            };

                            if rc.exists() {
                                println!("{rc:?}");
                                rc_exe_paths.push(rc);
                            }

                            if let Ok(bin) = path.join("bin").read_dir() {
                                for entry in bin.filter_map(Result::ok) {
