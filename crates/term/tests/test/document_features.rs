...
struct Fixture {
    app: Application,
    callbacks: mpsc::Receiver<(Feature, EditorCallback)>,
}

fn sender(feature: Feature, tx: &mpsc::Sender<(Feature, EditorCallback)>) -> EditorCallbackSender {
    let tx = tx.clone();
    ...
    EditorCallbackSender::new(
        move |callback| {
            let tx = tx.clone();
            async move {
                let _ = tx.send((feature, callback)).await;
            }
        },
        move |callback| event::send_blocking(&blocking, (feature, callback)),
    )
}
...
impl Fixture {
    ...
    async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        // pumping it until all four features have queued their first responses.
...
async fn requests_events_and_results_stay_with_each_editor() -> anyhow::Result<()> {
    ...
    second.edit("another");
    for fixture in [&mut first, &mut second] {
        // The first batch schedules work; the second publishes server responses.
        let requests = fixture.batch(ALL).await?;
        fixture.apply(requests);
...
async fn queued_highlights_follow_the_selection_and_view_document() -> anyhow::Result<()> {
    ...
    assert!(doc.document_highlights(view.id).is_none());
    let requests = fixture.batch(&[Feature::Highlights]).await?;
    fixture.apply(requests);
...
async fn remaining_servers_refresh_colors_and_links_after_an_exit() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            while let Ok((_, callback)) = fixture.callbacks.try_recv() {
