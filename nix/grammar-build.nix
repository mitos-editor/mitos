...
lib.extendMkDerivation {
  ...
  extendDrvArgs =
    ...
    in
    {
      pname = "mitos-tree-sitter-${name}";
      version = source.rev;

      # Avoid using fetchgit, and do not use fetchtree.
      # fetchtree is only avaliable if flakes are enabled.
      # Both are a bit jank (fetchgit is implemented via fetchtree these days) and will spam the terminal every time Mitos is built.
      ...
      src =
        if lib.hasPrefix "https://github.com" source.git then
          builtins.fetchurl {
            # Template:
            #   https://github.com/mitos-editor/mitos/archive/43bf7c2dc219606c64003aef21151f49f48d0939.tar.gz
